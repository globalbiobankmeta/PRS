...
option_list <- list(
  ...
  make_option(c("--prsfile"), type = "character", default = NULL, help = "Full path to a *.profile generated by Plink1.* or *.sscore by Plink2.*, note using --sum to get the SCORESUM for Plink 1.* or cols=+scoresums to get SCORE1_SUM for Plink 2.* used in this script"),
...
if(!is.null(pcfile)){
  pcs <- fread(pcfile, select = c("FID", "IID", paste0("PC", 1:10)))
  if(nrow(pcs) == 0){
...
res <- data.frame(prefix, pheno, pop, N, K, P, 
